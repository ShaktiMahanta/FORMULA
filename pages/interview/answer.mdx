...
| :---------------- | :------------------------------------- | :------------------------------------------------------ |
| Structure         | Single,Unified/togeather codebase      | Multiple independent services                           |
...
`@Transactional` annotation in Java, commonly used in the context of Spring framework for managing transactions,

- **Purpose**: It demarcates/separate transaction boundaries, ensuring that a `method` **or** `class` is executed within a database transaction. If any part of the method fails, the transaction can be rolled back, ensuring `data integrity`.
- **Usage**: You can apply it to methods or classes to define transactional behavior.
- **Configuration**: Typically, you'll use it in Spring applications with the @EnableTransactionManagement annotation or XML configuration.
...
**Q.How it works and what will happen in `DB` if it fails.**

**Apache Kafka** is an open-source distributed event streaming platform used for building `real-time data pipelines and streaming` applications. Here's a brief overview of how it works:

**How Apache Kafka Works**:

- **Producers**: Applications that publish data to Kafka.

- **Topics**: Kafka stores data in topics, which are divided into partitions for scalability.
...
**Failure Impact on Database:**
If Kafka fails, the impact on the database depends on how Kafka is integrated into your system. Here are some potential scenarios:

1. **Data Loss**: If Kafka is used for data ingestion and there's no backup, data that hasn't been processed might be lost.
2. **Application Downtime**: Applications relying on Kafka for real-time data might experience downtime or degraded performance.
3. **Data Inconsistency**: If Kafka is part of a distributed system, failure might lead to data inconsistency until the system recovers.
   To mitigate these risks, it's crucial to have redundancy, backups, and monitoring in place. This ensures high availability and quick recovery in case of failures

### Duplicate column count

```sql filename="sql" copy
...
- Use Encapsulation: make all your fields `private` and `final`
...
- The SOLID principles are a set of design principles in object-oriented programming that help create more `understandable`, `flexible`, and `maintainable` code. These principles were introduced by Robert C. Martin, also known as Uncle Bob. Here are the SOLID principles in Java:

1. **Single Responsibility Principle (SRP):**

   - A class should have only one job or responsibility.
   - **Example 1:** A class responsible for handling user authentication should not be responsible for handling database operations.
   - **Example 2: Banking System:**
     - Different Bank Services (Instead of creating them in one service, divide them into smaller services):
       - **BankServices** (e.g. deposit, withdrawal)
       - **LoanServices**
       - **NotificationService**
       - **PrinterService**
...
2. **Open/Closed Principle (OCP):**
   ...
   - **Example 1:** You can extend the functionality of a class through inheritance or interfaces without modifying the existing code.
   - **Example 2: NotificationService:**
     - Instead of using a tightly coupled implementation, declare a `NotificationService` interface with two methods (`sendOtp()` and `sendTransactionalDetails()`).
     - Implement the following services to handle different types of notifications:
...
3. **Liskov Substitution Principle (LSP)**:
   ...
   - **Example 1**: If `Bird` is a superclass and `Penguin` is a subclass, a `Penguin` should be able to replace a `Bird` object without breaking the program.
   - **Example 2: SocialMedia:**

     - There are different features provided by social media (like `chatWithFriend`, `GroupVideoCalling`, `sendVideoAndPhotos`, `publishAPost`, etc.):
       - **Facebook**: All features are applicable.
       - **Instagram**: `GroupVideoCalling` is not possible.
       - **WhatsApp**: `publishAPost` is not possible.

   - **Note**: To maintain the principle, create 3 interfaces and call them:
     1. `PublishMediaManager`: For methods related to publishing media, like `publishAPost`.
     2. `SocialMedia`: For methods like `chatWithFriend` and `sendVideoAndPhotos`.
     3. `VideoCallManager`: For methods related to video calling, like `GroupVideoCalling`.

4. **Interface Segregation Principle (ISP)**:

   - Clients should not be forced to depend on interfaces they do not use.
   - **Example**: Instead of having one large interface with many methods, create smaller, more specific interfaces.
   - **Example 2: UPI Payment System**:
     - **PhonePe**
     - **GooglePay**: Cashback features are available, so create a separate interface specifically for cashback functionality.
     - **Paytm**

5. **Dependency Inversion Principle (DIP)**:
   - High-level modules should not depend on low-level modules. Both should depend on abstractions (e.g., interfaces).
   - Abstractions should not depend on details. Details should depend on abstractions.
   - **Example 1**: Instead of a class directly instantiating another class, it should depend on an interface that the other class implements.
   - **Example 2: ShoppingMall**:
     - Create an interface `BankCard` that performs the transaction (`doTheTransaction` method), which can be implemented by either `DebitCard` or `CreditCard` classes. The decision about which class to use will be made at runtime, ensuring the system remains loosely coupled.

### Queue Vs Stack

### nth Highest salary sql

1. Using `LIMIT` with `OFFSET` (MySQL, PostgreSQL, etc.)
...
2. Using `DENSE_RANK()` (SQL Server, Oracle, PostgreSQL, MySQL 8+)
...
- A Functional Interface in Java is an interface with just one abstract method. These interfaces can be represented using lambda expressions, method references, or constructor references, making them a key part of functional programming in Java. Functional interfaces are annotated with the @FunctionalInterface annotation, although it's optional.
...
- **Examples of Functional Interfaces**:
  ...
  ```java filename="java" copy
        ...
        public class Main {
            public static void main(String[] args) {
                MyFunctionalInterface funcInterface = () -> System.out.println("Executing functional interface method.");
  ...
  ### Key Takeaways: When to Use Constructors vs. Setters

  | **Scinario**                                                                         | **Use Constructor** | **Use Setters** |
  | :----------------------------------------------------------------------------------- | :------------------ | :-------------- |
  | Creating an immutable object (e.g., Product, User Registration, Banking Transaction) | ✅ Yes              | ❌ No           |
  | Object must always have required values (e.g., ID, Name)                             | ✅ Yes              | ❌ No           |
  | Object data should not change after creation                                         | ✅ Yes              | ❌ No           |
  | Need to update values later (e.g., Order Status, User Profile)                       | ❌ No               | ✅ Yes          |
  | Some fields are optional                                                             | ❌ No               | ✅Yes           |

### ACID Properties

| **Property** | **Description**                                                   | **Example**                                                              |
| :----------- | :---------------------------------------------------------------- | :----------------------------------------------------------------------- |
| Atomicity    | All operations in a transaction succeed or fail as a single unit. | Money transfer: both debit and credit must be succeed or both must fail. |
| consistency  | Transaction bring the database from one valid state to another.   | Account balances cannot go negative.                                     |
| Isolation    | Concurrent transaction do not interfere with each other.          | Two transfers happening at the same time do not corrupt the data.        |
| Durability   | Committed transactions are permanent and survive system failures. | After a transfer, the changes are saved even if the system crashes.      |

- **Importance of ACID**:
  - **Data Integrity**: Ensures that data remains accurate and consistent.
  - **Reliability**: Guarantees reliability, even in the face of errors or failures.
  - **Trust**: Enables users and applications to trust the database to maintain correct and consistent data.
